  ...
  return (
    <section className="px-6 py-16">
      <div ref={ref} className={`mb-8 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
        <span className="text-[10px] tracking-[3px] text-[#88292F] uppercase block mb-2">sélection</span>
        <h2 className="font-cormorant text-[2.8rem] font-medium leading-none text-[#2e1e0f]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Pièces<br />Tendances</h2>
      </div>
...
function AllProductsSection({ products, productPairs, loading }) {
  const { ref, isVisible } = useScrollReveal()
  return (
    <section className="px-5 pb-20">
      <div ref={ref} className={`mb-8 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
        <span className="text-[10px] tracking-[3px] text-[#88292F] uppercase block mb-2">catalogue</span>
        <h2 className="font-cormorant text-[2.8rem] font-medium leading-none text-[#2e1e0f]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Toutes les<br />Pièces</h2>
      </div>
      {loading ? (
        <div className="text-center py-16">
          <div className="w-6 h-6 border-2 border-[#88292F] border-t-transparent rounded-full animate-spin mx-auto" />
        </div>
      ) : products.length === 0 ? (
        <div className="text-center py-16">
          <p className="font-cormorant text-2xl text-[#88292F]">Aucune pièce trouvée</p>
        </div>
      ) : (
  ...
  return (
    <section className="px-5 pb-8">
      <div ref={ref} className={`mb-6 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
        <span className="text-[10px] tracking-[3px] text-[#88292F] uppercase block mb-2">notre boutique</span>
        <h2 className="font-cormorant text-[2.4rem] font-medium leading-none text-[#2e1e0f]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Venez nous<br />rendre visite</h2>
      </div>
      <div className="bg-white/60 backdrop-blur-sm rounded-[24px] p-5 mb-4 shadow-sm">
        <div className="flex items-start gap-3 mb-4">
          <div className="w-10 h-10 rounded-full bg-[#88292F]/10 flex items-center justify-center flex-shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#88292F" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
          </div>
          <div>
        ...
        <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=Bonjour! Je souhaite prendre rendez-vous pour visiter votre boutique.`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-3 bg-[#2e1e0f] text-white rounded-full text-[13px] tracking-[1px] hover:bg-[#88292F] transition-colors duration-300">
      ...
      <div className="rounded-[24px] overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.08)] border border-[#e8e0d8]">
        <iframe src={MAP_EMBED_URL} width="100%" height="280" style={{ border: 0, filter: 'grayscale(0.3) sepia(0.1)' }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Localisation LILOOK" className="w-full" />
      </div>
    </section>
  )
}
...
function MostLikedSection({ products }) {
  const { ref, isVisible } = useScrollReveal()
  return (
    <section className="px-6 py-16 bg-[#e5c5a4]/5 border-y border-[#e5c5a4]/10">
      <div ref={ref} className={`mb-8 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
        <span className="text-[10px] tracking-[3px] text-[#c8a27b] uppercase block mb-2">Sélection populaire</span>
        <h2 className="font-cormorant text-[2.8rem] font-medium leading-none text-[#2e1e0f]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Les Plus<br />Aimés</h2>
      </div>
      <div className="flex gap-4 overflow-x-auto pb-4 scroll-snap-x mandatory hide-scrollbar">
        {products.map((product) => (
          <div key={product.id} className="flex-shrink-0 w-[260px] scroll-snap-start">
