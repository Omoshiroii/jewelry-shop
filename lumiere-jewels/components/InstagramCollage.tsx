...
export default function InstagramCollage() {
  ...
  return (
    <section className="py-16 md:py-20 overflow-hidden">
      <div ref={titleRef} className={`px-4 md:px-6 mb-8 flex items-end justify-between transition-all duration-700 ${titleVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
        <div>
      ...
      <div className="relative px-3 md:px-4">
        <div className="grid grid-cols-12 grid-flow-dense gap-1 md:gap-1.5 auto-rows-[100px] md:auto-rows-[130px]">
          {images.map((img, index) => (
            <CollageImage key={index} image={img} index={index} />
          ))}
          <div className="col-span-4 row-span-2 bg-[#2e1e0f] rounded-[12px] md:rounded-[16px] flex flex-col items-center justify-center p-3 md:p-4 text-center">
            ...
            <p className="font-cormorant text-[16px] md:text-[18px] text-white font-medium mb-1 mt-2 md:mt-3" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Rejoignez-nous</p>
            <p className="text-[10px] md:text-[11px] text-white/60 mb-3 md:mb-4">@lilook51</p>
            <a href="https://instagram.com/lilook51/" target="_blank" rel="noopener noreferrer" className="px-4 md:px-5 py-1.5 md:py-2 bg-white/10 border border-white/20 rounded-full text-[10px] md:text-[11px] text-white tracking-wide hover:bg-white/20 transition-colors">Suivre</a>
          </div>
        </div>
      </div>
    </section>
...
function CollageImage({ image, index }: { image: typeof images[number]; index: number }) {
  const { ref, isVisible } = useScrollReveal({ threshold: 0.1 })
  const gridClasses = { tall: 'col-span-3 row-span-2', wide: 'col-span-5 row-span-1', square: 'col-span-3 row-span-1' }
  return (
    <div ref={ref} className={`${gridClasses[image.size]} rounded-[12px] md:rounded-[16px] overflow-hidden group transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: `${index * 80}ms` }}>
      <a href="https://instagram.com/lilook51/" target="_blank" rel="noopener noreferrer" className="block w-full h-full relative">
